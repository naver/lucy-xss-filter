...
public class XssFilterSaxSimpleTest extends XssFilterTestCase {
	...
	public void testEndTagFilter() {
		...
		System.out.println("dirty : " + dirty);
		System.out.println("clean : " + clean);
		assertNotSame(unexpected, clean);
	}
	...
	public void testRemoveAllAttributesTest() {
		...
		assertEquals(expected.replace("</iframe>", ""), clean);
	...
	public void testIEHackTagWithoutCloseTag() {
		...
		assertEquals(expected, clean); // end tag 검사하지 않아 필터링 없이 그데로 출력됨.
	...
	public void urlEncodingData() {
		...
		String clean = filter.doFilter(valid);
		assertEquals("http://m.id.hangame.com/searchInfo.nhn?type=FINDID&nxtURL=http://m.tera.hangame.com&lt;/script&gt;<!-- Not Allowed Attribute Filtered ( onerror=alert(/V/)) --><img src=pooo.png>" , clean);
	...
	public void withoutCommentMultiThreadTest() {
		...
		while(keys.hasMoreElements()) {
			...
			if (nextKey%2 == 0) { // 짝수면 주석이 있어야함
				assertEquals(expectedWithComment, result.get(nextKey));
			} else { // 홀수면 주석 생략되어야함.
				assertEquals(expectedWithoutComment, result.get(nextKey));
			}
		}
	}
	...
	public void variousInputMultiThreadTest() {
		...
		while(keys.hasMoreElements()) {
			Integer nextKey = keys.nextElement();
			assertEquals(expectedString[nextKey % targetString.length], result.get(nextKey));
		}
	}
	...
	public void variousInputVariousConfigMultiThreadTest() {
		...
		while(keys.hasMoreElements()) {
			Integer nextKey = keys.nextElement();
			assertEquals(expectedStringOnOtherConfig[nextKey % targetStringOnOtherConfig.length], result.get(nextKey));
		}
	}
	...
	public void charArrayInput() {
		...
		assertNotSame(expected, writer.toString());
	}
